...
def write_model_flatbuffer(output_dir, tflite_model_path):
    ...
    model_name = os.path.basename(tflite_model_path).split('.')[0]  # e.g., toycar_int8.tflite -> toycar_int8
    model_data_name = f"{model_name}_model_data"  # e.g., toycar_int8 -> toycar_int8_model_data
    model_data_path = os.path.join(output_dir, f"{model_data_name}.cc")  # e.g., toycar_int8_model_data.cc

    # Flatbuffer
    # https://ai.google.dev/edge/litert/microcontrollers/build_convert#convert_to_a_c_array
